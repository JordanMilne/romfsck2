...
fatal_errors = set((ERR_MAGIC, ERR_SIZE, ERR_HEADER))
...
def read_romfs_inode(addr, parent_inode, ctx):
    ...
    try:
        ...
        if inode.type in set((FS_FIFO, FS_FILE, FS_SOCKET, FS_SYMLINK)):
            if inode.spec_info:
        ...
        if inode.type in set((FS_CHAR_DEV, FS_FIFO)):
            if inode.executable:
        ...
        if inode.type not in set((FS_FILE, FS_SYMLINK)):
            if inode.file_size:
